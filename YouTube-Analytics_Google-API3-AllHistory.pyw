...
# Ensure google-api-python-client is installed and importable
try:
    from googleapiclient.discovery import build
except ImportError:
    import subprocess, sys as _sys
    subprocess.check_call([_sys.executable, '-m', 'pip', 'install', 'google-api-python-client'])
    from googleapiclient.discovery import build
...
# Load API key from Python file
try:
    creds = runpy.run_path(api_key_file)
    API_KEY = creds.get('YOUTUBE_DATA_API_KEY')
except FileNotFoundError:
    print(f"Error: API key file not found at {api_key_file}")
    sys.exit(1)
if not API_KEY:
    print("Error: YOUTUBE_DATA_API_KEY not defined in key file.")
    sys.exit(1)

channel_id = 'UC0Zoz9yO4DbkaRf6cZ9iPTw'  # Your channel ID
max_results_per_page = 50  # Adjust as needed
num_latest_videos = 150      # Number of latest videos to fetch details for
...
def main():
    ...
    videos_data = []
    if latest_video_ids:
        # Batch the video IDs to avoid exceeding API limits
        batch_size = 50  # YouTube API limit for 'id' parameter
        for i in range(0, len(latest_video_ids), batch_size):
            batch_ids = latest_video_ids[i:i + batch_size]
            videos_request = youtube.videos().list(
                part="snippet,statistics,contentDetails,topicDetails,liveStreamingDetails,localizations",
                id=','.join(batch_ids)
            )
            videos_response = videos_request.execute()
            videos_data.extend(videos_response.get("items", []))
    ...
    csv_path = f'D:/Documents/_Data-Vault/YouTube-Stats/video-details_{csv_date}_all.csv'
