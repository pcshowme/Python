...
def get_top_videos(youtube_analytics):
    ...
    if response and 'rows' in response:
        ...
        video_details_request = youtube_data_service.videos().list(
            part='snippet',
            id=','.join(video_ids)
        )
...
if __name__ == '__main__':
    ...
    with open(OUTPUT_CSV_FILE, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Timestamp', 'Channel Title', 'Description', 'Published Date', 'Subscriber Count', 'Total Views', 'Total Videos', 'Channel Keywords',
